...
class MindMapSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			.setName('Debug mode')
			.setDesc('Toggle verbose logging. Tap the root node of a mindmap to export logs to the clipboard.')
			.addToggle(toggle => toggle
				...
				.onChange(async (value) => {
					...
					new Notice(value
						? 'Debug mode enabled. Tap the root node in a mindmap to copy logs to clipboard.'
						: 'Debug mode disabled.');
				}));
	}
}